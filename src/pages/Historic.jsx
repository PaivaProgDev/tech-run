import { Calendar, Flame, History, Timer } from "lucide-react"
import { useAuth } from "../contexts/AuthContext"
import ViewPort from "../components/Global/ViewPort"

const Historic = () => {
    const { races, data } = useAuth()
                        ...
                        <div className="flex flex-col gap-4">
                            {
                                data ? (
                                    data.map((info) => (
                                        <div key={info.id} className="w-full flex justify-center flex-wrap gap-10">
                                            {
                                                info.data().isConcluded ? (
                                                    <div className={` p-4 rounded-md bg-white w-full flex flex-col max-w-[20rem] shadow-md`}>
                                                        <div className="flex justify-end">
                                                            {info.data().isConcluded ? <div className="h-2.5 w-2.5 bg-green-500 rounded-full"></div> : <div className="h-2.5 w-2.5 bg-red-500 rounded-full"></div>}
                                                        </div>
                                                        <div className="text-center">
                                                            <pre className="font-semibold text-[var(--color-3)]">{info.data().distance} km</pre>
                                                        </div>
